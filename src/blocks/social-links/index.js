...
const socialLinks = registerBlockType( 'pcc/social-links', {
	...
	styles: [
		...
		{
			name: 'icon-only',
			label: __( 'Icon Only', 'pcc-framework' ),
		},
	],
	edit: ( { attributes, setAttributes, className, isSelected } ) => {
				...
				icon: (
					...
					>
						<Path
							...
							d="M18.03,31.02h3.934V21.5h2.624l.348-3.281H21.964l0-1.643c0-.855.082-1.314,1.309-1.314h1.64V11.98H22.292c-3.153,0-4.262,1.592-4.262,4.268v1.97H16.064V21.5H18.03ZM20.5,39A17.5,17.5,0,1,1,38,21.5,17.5,17.5,0,0,1,20.5,39Z"
						/>
					</SVG>
				),
			},
		...
		const onChangeLabelFaceBook = ( value ) => {
			setAttributes( {
				label_facebook: value
					? value
					: socialLinks.attributes.label_facebook.default,
			} );
		};
		...
		const onChangeLabelTwitter = ( value ) => {
			setAttributes( {
				label_twitter: value
					? value
					: socialLinks.attributes.label_twitter.default,
			} );
		};
